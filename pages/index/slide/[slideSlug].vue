<template>
  <section @keyup.esc="navigateTo(`/#${slide.slug}`)" class="fixed inset-0 flex flex-col items-center justify-center backdrop-blur">
    <article ref="mainSection" tabindex="-1" @keyup.left="goToPrevSlide" @keyup.right="goToNextSlide" class="slide p-6 md:p-10 lg:p-12 bg-woes-dark border-8 border-woes-light w-screen h-screen md:w-[80vw] md:max-h-[600px] md:max-w-[1200px] grid grid-cols-1 md:grid-cols-[1fr_2fr] md:grid-rows-[32px,minmax(0,1fr),32px] gap-y-4 md:gap-6">
      <NuxtLink :to="`/#${slide.slug}`" class="col-span-2 block text-right">
      ...
      <figure class="row-start-2">
        <img
          ...
          height="300"
          width="300"
          class="mx-auto h-60 max-w-full md:h-auto"
      ...
      <div class="row-start-3 md:row-start-2 grid grid-cols-2 auto-rows-min gap-4 overflow-y-auto">
        ...
        <h3 class="text-2xl uppercase flex gap-4 col-span-2">
          <span>{{ slide.acronym }}</span>|<span>{{ slide.type.replace('-', ' ') }}</span>
        </h3>
      ...
      <footer class="col-span-2 flex justify-end gap-4">
        <div class="col-span-2 block text-right w-8">
          <NuxtLink v-if="prevSlide" :to="`/slide/${prevSlide}`" class="w-full">
            <ion-icon name="arrow-back-outline" size="large"></ion-icon>
          </NuxtLink>
        </div>
        <div class="col-span-2 block text-right w-8">
          <NuxtLink v-if="nextSlide" :to="`/slide/${nextSlide}`" class="w-full">
